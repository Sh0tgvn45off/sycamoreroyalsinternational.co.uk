/* =================================================
   EVENTS PAGE - INTERACTIVE SCROLL ANIMATION
   Handles 4 program sections (200vh each)
   ================================================= */

// Only run on Events page with scroll wrapper
if (document.querySelector('.about-scroll-wrapper')) {
  ...
  // Handling scroll for the program tabs - 4 SECTIONS
  const sections = document.querySelectorAll('.about-tab-content');
  const visuals = document.querySelectorAll('.about-tab-visual');
  const scrollMultiplier = window.innerHeight * 2; // 2x viewport height per section (200vh) the last index to compute the limit 
  ...
  function updateActiveSection() {
    ...
    // Keep last section active until user scrolls way past
    if (adjustedScroll > (lastIndex + 0.5) * scrollMultiplier) {
      sections[lastIndex].classList.add('is-active');
      if (visuals[lastIndex]) {
        visuals[lastIndex].classList.add('is-active');
      }
    }
  }
  ...
  if (window.location.search.includes('debug')) {
    document.addEventListener('scroll', () => {
      ...
      console.log('Scroll:', scrollPos, 'Program Section:', currentSection);
    });
  }
}